...
import com.x2mobile.wodjar.activity.PersonalRecordResultsActivity
...
class PersonalRecordFragment : BaseFragment(), PersonalRecordListener {
    ...
    override fun onPersonalRecordClicked(personalRecord: PersonalRecord) {
        if (Preference.isLoggedIn(context)) {
            if (personalRecord.present) {
                startActivity(context.intentFor<PersonalRecordResultsActivity>(NavigationConstants.KEY_PERSONAL_RECORD to personalRecord))
            } else {
